...
MicroAbundance$continuousDischarge <- NA
for(i in 1:nrow(MicroAbundance)){
  maxTime <- as.POSIXct(MicroAbundance$collectDate.x[i])+(5*60)
  minTime <- as.POSIXct(MicroAbundance$collectDate.x[i])-(5*60)
  currSite <- MicroAbundance$siteID[i]
  meanQ <- mean(csd_continuousDischarge$maxpostDischarge[csd_continuousDischarge$endDate>=minTime
                                                         &csd_continuousDischarge$endDate<maxTime
                                                         &csd_continuousDischarge$siteID==currSite
                                                         &csd_continuousDischarge$dischargeFinalQF==0],
                na.rm = T)
  MicroAbundance$continuousDischarge[i] <- meanQ
}
MicroAbundance$continuousDischarge[is.nan(MicroAbundance$continuousDischarge)] <- NA
